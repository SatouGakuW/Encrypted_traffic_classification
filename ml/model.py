...
class CNN(LightningModule):
    ...
    def train_dataloader(self):
        ...
        print("model",self.hparams.data_path)
        # dataset_dict = datasets.load_dataset(data_files=[self.hparams.data_path])
        path_path = Path(self.hparams.data_path+'processed_train_data')
        train_list=[]
        for path in path_path.iterdir():
            if '.parquet' not in str(path):
                continue
            train_list.append(str(path))
        print("path",train_list,type(train_list))
        path_path = Path(self.hparams.data_path+'processed_test_data')
        test_list=[]
        for path in path_path.iterdir():
            if '.parquet' not in str(path):
                continue
            test_list.append(str(path))
        print("path",test_list,type(test_list))

        dataset_dict = datasets.load_dataset('parquet',data_files={'train':train_list,'test':test_list})
